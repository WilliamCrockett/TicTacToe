...
var gameCollection = void 0;

var numWins = 0;

var numGamesPlayed = 0;

var numGamesFinsihed = 0;

var completionPercentage = 0;

var clickedID = function clickedID(event) {
...
var getTotalGames = function getTotalGames(data) {
  numGamesPlayed = 0;
  numGamesFinsihed = 0;
  gameCollection = data.games;
  generateNumGames();
...
var generateNumGames = function generateNumGames() {
  for (var i = 0; i < gameCollection.length; i++) {
    numGamesPlayed++;
...
var getCompletedGames = function getCompletedGames() {
  api.getGames().then(getData);

  getAllGames();
};

var getAllGames = function getAllGames() {
  api.getAllGames().then(getTotalGames);
};

var onBlockSelect = function onBlockSelect() {
...
var onStartNewGame = function onStartNewGame() {
  ...
  api.initGame().then(ui.newGame).catch(ui.newGameError); //  TODO Update this to real error message
};

var onDismissAlert = function onDismissAlert() {
  ui.dismissAlert();
};
...
module.exports = {
  ...
  generateNumGames: generateNumGames
};
...
var userX = 0;
var userO = 0;
var winningCombo = void 0;

var incWinner = function incWinner(winningUser) {
  if (winningUser === 'X') {
    userX++;
    $('#userX').text(userX);
...
var placeX = function placeX(targetID) {
  $('#' + targetID).text('X');
  $('#' + targetID).css('cursor', 'not-allowed');
};

var placeO = function placeO(targetID) {
  $('#' + targetID).text('O');
  $('#' + targetID).css('cursor', 'not-allowed');
};

var showWinner = function showWinner(win, combo) {
  for (var i = 0; i < 3; i++) {
...
var newGameError = function newGameError() {
  $('#cantPlay').toggle(); // update to new banner
};

var updateStats = function updateStats(wins) {
  $('#previous-game-stats').text("You've won a total of " + wins + ' games in your career');
};

var updateStatsComplete = function updateStatsComplete(completeGames) {
  $('#hor-rule').toggle();
  $('#all-game-stats').text("You've completed " + completeGames + '% of games started');
};

var spacePlayed = function spacePlayed() {
  $('#cantPlay').toggle();
};
...
module.exports = {
  ...
  spacePlayed: spacePlayed,
  dismissError: dismissError,
  newGameError: newGameError,
  gameOver: gameOver,
  dismissAlert: dismissAlert,
  updateStatsComplete: updateStatsComplete,
...
var finishGame = function finishGame(over) {
  return $.ajax({
    url: config.apiOrigin + '/games/' + store.gameData.id,
    method: 'PATCH',
    headers: {
...
var onChangePassword = function onChangePassword(event) {
  event.preventDefault();
  var data = getFormFields(this);
  api.changePassword(data).then(ui.onPasswordChangeSuccess).catch(ui.onPasswordChangeFailure);
};
...
var clearFields = function clearFields(event) {
  event.preventDefault();
  $('#signInEmail').val('');
  $('#signInPassword').val('');
  $('#signUpEmail').val('');
...
var onPasswordChangeSuccess = function onPasswordChangeSuccess(data) {
...
var onPasswordChangeFailure = function onPasswordChangeFailure(data) {
  $('#changePasswordModal').modal('hide');
  $('#signUpEvent').modal('show');
  $('#signUpTitle').text('Password ');
...
var switchTurn = function switchTurn(player) {
  if (player === 'X') {
    currentPlayer = 'O';
...
var checkSpace = function checkSpace(boardPosition) {
  var location = parseInt(boardPosition);
  if (game.gameCells[location] !== '') {
    return true;
  }
};
// plays as user x
var playerXGo = function playerXGo(boardPosition) {
  var location = parseInt(boardPosition);
...
// plays as user O
var playerOGo = function playerOGo(boardPosition) {
  var location = parseInt(boardPosition);
...
var tempCheck = function tempCheck(gameCells) {
...
var checkWinner = function checkWinner(gameCells) {
  var count = 0;
  ...
  if (gameCells[0] !== '' && gameCells[0] === gameCells[3] && gameCells[3] === gameCells[6]) {
    ...
    events.winner = gameCells[0];
    api.finishGame('true').then(store.gameData.over = true);
    return events.winner;
  } else if (gameCells[1] !== '' && gameCells[1] === gameCells[4] && gameCells[4] === gameCells[7]) {
    ui.winningCombo = [1, 4, 7];
    events.winner = gameCells[1];
    api.finishGame('true').then(store.gameData.over = true);
    return events.winner;
  } else if (gameCells[2] !== '' && gameCells[2] === gameCells[5] && gameCells[5] === gameCells[8]) {
    ui.winningCombo = [2, 5, 8];
    events.winner = gameCells[2];
    api.finishGame('true').then(store.gameData.over = true);
    return events.winner;
  } else if (gameCells[0] !== '' && gameCells[0] === gameCells[1] && gameCells[1] === gameCells[2]) {
    ui.winningCombo = [0, 1, 2];
    events.winner = gameCells[0];
    api.finishGame('true').then(store.gameData.over = true);
    return events.winner;
  } else if (gameCells[3] !== '' && gameCells[3] === gameCells[4] && gameCells[4] === gameCells[5]) {
    ui.winningCombo = [3, 4, 5];
    events.winner = gameCells[3];
    api.finishGame('true').then(store.gameData.over = true);
    return events.winner;
  } else if (gameCells[6] !== '' && gameCells[6] === gameCells[7] && gameCells[7] === gameCells[8]) {
    ui.winningCombo = [6, 7, 8];
    events.winner = gameCells[6];
    api.finishGame('true').then(store.gameData.over = true);
    return events.winner;
  } else if (gameCells[0] !== '' && gameCells[0] === gameCells[4] && gameCells[4] === gameCells[8]) {
    ui.winningCombo = [0, 4, 8];
    events.winner = gameCells[0];
    api.finishGame('true').then(store.gameData.over = true);
    return events.winner;
  } else if (gameCells[2] !== '' && gameCells[2] === gameCells[4] && gameCells[4] === gameCells[6]) {
    ui.winningCombo = [2, 4, 6];
    events.winner = gameCells[2];
    api.finishGame('true').then(store.gameData.over = true);
    return events.winner;
  }
